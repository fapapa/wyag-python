...
def hash_object(fd, fmt, repo=None):
    ...
    # Choose constructor based on object type found in header
...
def kvlm_parse(raw, start=0, dct=None):
    if not dct:
        ...
        # You CANNOT declare the argument as dct=OrderedDict() or all calls to
...
def tree_parse_one(raw, start=0):
    ...
    # Find thu null terminator of the path
    y = raw.find(b'\x00', x)
    # and read the path
    path = raw[x+1:y]
    ...
    sha = hex(
        int.from_bytes(
            raw[y+1:y+21], "big"))[2:] # hex adds 0x in front; we don't want
                                       # that
...
def cmd_ls_tree(args):
    ...
    for item in obj.items:
        print("{0} {1} {2}\t{3}".format(
            ...
            # Git's ls-tree displays the type of the object pointed to. We can
            # do that too :)
            object_read(repo, item.sha).fmt.decode("ascii"),
            item.sha,
            item.path.decode("ascii")))

argsp = argsubparsers.add_parser("checkout", help="Check out a commit inside of a directory.")
